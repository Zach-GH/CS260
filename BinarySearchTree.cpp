...
//Created Nodes and designated pointers
struct Node {
    Bid bid;
    Node* parentPtr;
    Node* leftPtr;
...
void BinarySearchTree::Insert(Bid bid) {
    ...
    //Checking to see if there are any pointers, setting pointers around encapsulated data
    //if there is data, then add node to root
    if (root == nullptr) {
        root = new Node;
        root->bid = bid;
...
void BinarySearchTree::Remove(string bidId) {
    ...
    //checking for pointers in list, if there are, change pointers and redirect structure before delete
...
Bid BinarySearchTree::Search(string bidId) {
    ...
    //checking pointers after search, return search answer if there are pointers
    Node* nodePtr = SearchNode(root, bidId);
    if (nodePtr == nullptr) {
        Bid bid;
        return bid;
...
void BinarySearchTree::addNode(Node* node, Bid bid) {
    ...
    //set basic parameters to encapsulate keys check if there is any data within the tree add and redirect pointer to the right or left
    int curKey = atoi(node->bid.bidId.c_str());
